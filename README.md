...
## Linear Regression Modeling to Predict MPG Performance Based on Multilple Car Characteristics
...
## Summary Statistics on Suspension Coil Performance Variance Across Sample Lots
...
When we perform our analysis on the lot dataset as an aggregate it would appear that the suspension coil standard for PSI variance is met - the measured variance of 62.69 is less than the allowable max of 100. 
...
When we break out the statistical data per Lot# we can see that the variance for Lot1 and Lot2 are well below the tolerance level for variance but that the variance for Lot3 is above the standard tolerance for variance. The cars belonging to Lot3 would not meet the standard as there is too much volatility in the data set for Coil PSI of these cars. Based on analysis our recommendation is that cars from Lot3 are pulled flagged for service and re-tested for safety conformance. 


## T-Tests on Suspension Coils for Mean PSI of Sample Set vs Population Set
### Measuring the potential statistical difference of our lots (summarized and individualized) against the sample population mean PSI of 1500.
...
- H0 : There is no statistical difference between the observed sample mean of PSI and the presumed population mean of 1500 PSI.
- Ha : There is a statistical difference between the observed sample mean PSI and the presumed population mean of 1500 PSI.

The lots in aggregate have a summarized p value of 0.06 (above our assume significance level of 0.05) which means that we cannot reject our null hypotheses and there is no statistical difference of our sample lots from the presumed popultion mean of 1500 PSI.
...
In order to perform analysis we need to collect the MSRP and MPG across all makes of cars currently on the market for models that have the same passenger capacity as the MechaCar

To perform analysis on our hypotheses, we would use multiple linear regression to determine the correlation and predictability of market-competitiveness based on our independent variables (MSRP and MPG).
